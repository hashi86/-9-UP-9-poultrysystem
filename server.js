...
// ==================== VACCINE ALERT CHECK ====================
const vacSchedulesServer = {
  broiler: [
    { day: 1,  name: 'ماريك + HVT + برونشيتيس H120', route: 'رش / قطرة عين' },
    { day: 7,  name: 'نيوكاسل B1 + IB (الجرعة 1)',    route: 'ماء الشرب / رش' },
    { day: 12, name: 'غامبورو IBD (الجرعة 1)',         route: 'ماء الشرب' },
    { day: 19, name: 'نيوكاسل + IB (الجرعة 2)',        route: 'ماء الشرب' },
    { day: 25, name: 'غامبورو IBD (الجرعة 2)',         route: 'ماء الشرب' },
    { day: 35, name: 'نيوكاسل (تنشيط - اختياري)',      route: 'ماء الشرب' },
  ],
  layer: [
    { day: 1,   name: 'ماريك + HVT',                        route: 'حقن تحت الجلد' },
    { day: 7,   name: 'برونشيتيس IB H120',                  route: 'رش / قطرة' },
    { day: 14,  name: 'غامبورو IBD',                        route: 'ماء الشرب' },
    { day: 21,  name: 'نيوكاسل B1 + IB',                   route: 'ماء الشرب' },
    { day: 35,  name: 'جدري الدجاج (POX)',                  route: 'وخز الجلد' },
    { day: 42,  name: 'كوريزا + مايكوبلازما',              route: 'حقن' },
    { day: 56,  name: 'نيوكاسل + IB (تنشيط)',              route: 'ماء الشرب' },
    { day: 70,  name: 'سالمونيلا',                          route: 'ماء الشرب' },
    { day: 112, name: 'نيوكاسل + IB + EDS (قبل الإنتاج)',  route: 'حقن - ضروري' },
  ],
  quail: [
    { day: 4,  name: 'نيوكاسل B1 (اختياري)',              route: 'قطرة عين' },
    { day: 12, name: 'جدري الطيور (مناطق موبوءة)',         route: 'وخز الجلد' },
    { day: 21, name: 'نيوكاسل (تنشيط - اختياري)',         route: 'ماء الشرب' },
  ]
};

app.get('/api/alerts/vaccine-check', (req, res) => {
  const sheds = db.prepare('SELECT * FROM sheds WHERE active=1 AND current_birds > 0').all();
  let created = 0;
  const results = [];
  for (const shed of sheds) {
    const schedule = vacSchedulesServer[shed.bird_type] || vacSchedulesServer['broiler'];
    const currentAge = shed.age_days || 0;
    const upcoming = schedule.filter(v => {
      const diff = v.day - currentAge;
      return diff >= 0 && diff <= 3;
    });
    for (const vac of upcoming) {
      const diff = vac.day - currentAge;
      const isToday = diff === 0;
      const severity = isToday ? 'danger' : diff <= 1 ? 'warning' : 'info';
      const timeLabel = isToday ? 'اليوم!' : diff === 1 ? 'غداً' : `بعد ${diff} أيام`;
      const title = `💉 تحصين ${shed.name}: ${vac.name}`;
      const dup = db.prepare("SELECT id FROM alerts WHERE title=? AND read=0 AND created_at >= datetime('now','-1 day')").get(title);
      if (!dup) {
        db.prepare('INSERT INTO alerts (type,title,message,severity) VALUES (?,?,?,?)').run(
          'vaccine',
          title,
          `${timeLabel} — يوم ${vac.day} · ${vac.route} · العنبر يضم ${shed.current_birds.toLocaleString('ar')} طير`,
          severity
        );
        created++;
      }
      results.push({ shed: shed.name, vaccine: vac.name, day: vac.day, diff, severity });
    }
  }
  res.json({ checked: sheds.length, created, upcoming: results });
});
...
app.get('/api/production/mortality-trend', (req, res) => {
  const sheds = db.prepare('SELECT * FROM sheds WHERE active=1 AND current_birds > 0').all();
  const rows = db.prepare(`
    SELECT shed, date, mortality, eggs
    FROM production_log
  ...
  const result = sheds.map(shed => {
    const shedRows = rows.filter(r => r.shed === shed.name);
    const thisWeek = [], lastWeek = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date(); d.setDate(d.getDate() - i);
      const dateStr = d.toISOString().split('T')[0];
      const row = shedRows.find(r => r.date === dateStr);
      thisWeek.push({ date: dateStr, mortality: row?.mortality || 0 });
    }
    for (let i = 13; i >= 7; i--) {
      const d = new Date(); d.setDate(d.getDate() - i);
      const dateStr = d.toISOString().split('T')[0];
      const row = shedRows.find(r => r.date === dateStr);
      lastWeek.push({ date: dateStr, mortality: row?.mortality || 0 });
    }
    const thisTotal = thisWeek.reduce((s, d) => s + d.mortality, 0);
    const lastTotal = lastWeek.reduce((s, d) => s + d.mortality, 0);
    const safeThresholdPerDay = Math.round(shed.current_birds * 0.005);
    const maxDaily = Math.max(...thisWeek.map(d => d.mortality), ...lastWeek.map(d => d.mortality), 1);
    return {
      name: shed.name,
      bird_type: shed.bird_type,
      current_birds: shed.current_birds,
      thisWeek,
      lastWeek,
      thisTotal,
      lastTotal,
      safeThresholdPerDay,
      maxDaily,
      trend: thisTotal > lastTotal ? 'up' : thisTotal < lastTotal ? 'down' : 'stable',
      hasDanger: thisWeek.some(d => d.mortality > safeThresholdPerDay)
    };
  });
  res.json(result);
});
...
const POULTRY_SYSTEM_PROMPT = `أنت خبير دواجن متخصص ومساعد ذكي لإدارة مزارع الدواجن. تجيب بالعربية بشكل احترافي ودقيق ومفصّل.
...
app.post('/api/ai/chat', async (req, res) => {
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      max_tokens: 2048,
      system: systemPrompt || POULTRY_SYSTEM_PROMPT,
      messages
    });
